...
class COP_modifying_Test < Test::Unit::TestCase
  ...
  def test_model_5_specific_air_loop
    ...
    path = OpenStudio::Path.new(File.dirname(__FILE__) + "/05_NewFreezer_Thermal_mass_20191212_out.osm")
    ...
    input_csv_path.setValue("./tests/Experiment5_200205_input.csv")
    # Run the measure
    ...
    model_out_path = File.join(File.dirname(path.to_s),File.basename(path.to_s,".osm")+"_out.osm")
