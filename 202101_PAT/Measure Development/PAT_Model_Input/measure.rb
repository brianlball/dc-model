...
require 'open3'
#start the measure
class ModelInput < OpenStudio::Measure::ModelMeasure
  ...
  def name
    ...
    def description
    return 'Clean seedFile and create all necessary input'
  end

  def modeler_description
    return 'purge all standard objects and add load inputs'
  end
  ...
  def run(model, runner, user_arguments)
    ...
    inside_convection_coff = runner.getDoubleArgumentValue("inside_convection_coff",user_arguments)
    interior_override = runner.getBoolArgumentValue("convection_override",user_arguments)
...
model.getSurfaces.each {|x| x.setWindExposure("No");x.setSunExposure("No")}
    ...
    convective_internal_gain_schedule_file = OpenStudio::Model::ScheduleFile.new(external_file,6+col_add,1)
 ...
 ## Assign scheduleFile to internal_gain 
    ...
    radiation_internal_gain_schedule_file = OpenStudio::Model::ScheduleFile.new(external_file,7+col_add,1)
...
if cooling_power
    ...
    cooling_power_schedule_file = OpenStudio::Model::ScheduleFile.new(external_file,5,1)
...
### ambient schedule create:
    ...
    site_wind_speed_output = output_var(model,weather_file,"Site Wind Speed","Site Wind Speed output variable")
      ...
      hvac_schedule_file = OpenStudio::Model::ScheduleFile.new(external_file,5+col_add,1)
      ...
      measure_period_schedule = OpenStudio::Model::ScheduleFile.new(external_file,8+col_add,1)
      ...
      output_var(model,measure_period_schedule,"Schedule Value")
...
# set plenum
if plenum_test
  plenum_temp_schedule_file = OpenStudio::Model::ScheduleFile.new(external_file,6,1)
...
## add surface_output and actuators
  ...
  surfaces.each do |surface| 
      ...
      # surface_outputs.each { |var| Os_lib.output_var(model,surface,var) }
      output_var(model,surface,"Surface Inside Face Convection Heat Transfer Coefficient")
      output_var(model,surface,"Surface Inside Face Convection Heat Gain Rate per Area")
      output_var(model,surface,"Surface Inside Face Temperature")
      output_var(model,surface,"Surface Outside Face Temperature")
  ...
  plenum_coff = inside_convection_coff*4
  surface_ems_program_enable_lines = []
  surface_ems_program_disable_lines = []
  cooling_space.surfaces.each do |surface| 
      ems_surface_inside_convection_actuator = OpenStudio::Model::EnergyManagementSystemActuator.new(surface,"Surface",
      "Interior Surface Convection Heat Transfer Coefficient")
      ems_surface_inside_convection_actuator.setName(surface.nameString+"_inside_convection_actuator")
      surface_ems_program_enable_lines += ["SET #{ems_surface_inside_convection_actuator.nameString} = #{inside_convection_coff},"]
      surface_ems_program_disable_lines += ["SET #{ems_surface_inside_convection_actuator.nameString} = Null,"]
  end
...
#   zone_mixing = OpenStudio::Model::ZoneMixing.new(cooling_space.thermalZone.get)
#   zone_mixing.setName(cooling_space.nameString+"_ZoneMixing")
#   zone_mixing.setSourceZone (gain_space.thermalZone.get)

#   zone_mixing = OpenStudio::Model::ZoneMixing.new(gain_space.thermalZone.get)
#   zone_mixing.setName(gain_space.nameString+"_ZoneMixing")
#   zone_mixing.setSourceZone (cooling_space.thermalZone.get)
# end
#   zone_mixing.setAirChangesperHour (52.0)
  # zone_mixing.setSchedule(hvac_schedule_file)
# set surface R

  gain_space.surfaces.each do |surface| 
    ems_surface_inside_convection_actuator = OpenStudio::Model::EnergyManagementSystemActuator.new(surface,"Surface",
    "Interior Surface Convection Heat Transfer Coefficient")
    ems_surface_inside_convection_actuator.setName(surface.nameString+"_inside_convection_actuator")
    surface_ems_program_enable_lines += ["SET #{ems_surface_inside_convection_actuator.nameString} = #{plenum_coff},"]
    surface_ems_program_disable_lines += ["SET #{ems_surface_inside_convection_actuator.nameString} = Null,"]
    # ems_surface_inside_convection_actuator = OpenStudio::Model::EnergyManagementSystemActuator.new(surface,"Surface",
    #   "Exterior Surface Convection Heat Transfer Coefficient")
    #   ems_surface_inside_convection_actuator.setName(surface.nameString+"_outside_convection_actuator")
    #   surface_ems_program_enable_lines += ["SET #{ems_surface_inside_convection_actuator.nameString} = #{plenum_coff},"]
    #   surface_ems_program_disable_lines += ["SET #{ems_surface_inside_convection_actuator.nameString} = Null,"]
end
...
if cooling_power
  surface_ems_program_lines +=  ["IF #{cooling_power_schedule_file_sensor.nameString} > 1.0,"]
...
ModelInput.new.registerWithApplication
