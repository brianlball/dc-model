...
class TimeseriesObjectiveFunction < OpenStudio::Measure::ReportingMeasure
  ...
  def arguments(model)
    ...
    thermalZone_names = model.getThermalZones.map {|x| x.nameString}
    if thermalZone_names.include?"Conditioned_Zone"
      zone_name = "Conditioned_Zone"
    elsif thermalZone_names.include?"ConditionedZone"
    ...
    csv_name.setDefaultValue("../../../lib/Experiment2_200114_input_out.csv")
    args << csv_name
    ...
    csv_var.setDefaultValue("ZoneTemperature")
    args << csv_var
    ...
    sql_key.setDefaultValue(zone_name)
    args << sql_key  
    ...
    sql_var.setDefaultValue("Zone Air Temperature")
    args << sql_var    
  ...
  def run(runner, user_arguments)
    ...
    runner.registerInfo("csv_name: #{csv_name} - csv_time_header #{csv_time_header}, csv_var - #{csv_var}, sql_key - #{sql_key}")


    ## use measure_meter_csv_path
    measure_meter_csv_path = OsLib_HelperMethods.check_upstream_measure_for_arg(runner, 'measure_meter_csv_path')
    
    if measure_meter_csv_path
      meter_file_path = measure_meter_csv_path[:value]
    else
      meter_file_path = File.expand_path(csv_name)
    end
    ...
    runner.registerInfo("csv_name: #{meter_file_path}")
    
    csv = CSV.read(meter_file_path, :encoding => 'ISO-8859-1')
    ...
    runner.registerInfo("final_avail: #{find_avail}")
    ...
    #export for plotting
    all_series = []
        # first_values = (0..10).map {|i| [ts.dateTimes(i),ts.values(i)].join(" : ")}
    
    runner.registerInfo("#{map.to_s} ")
    csv[0].each do |hdr|
      runner.registerInfo("#{hdr.to_s} - #{csv_time_header.to_s} - #{(hdr.to_s != csv_time_header.to_s)} - #{map.keys}   - #{!map.key? hdr}")
      ...
      if (hdr.to_s != csv_time_header.to_s)
        ...
        series2["color"] = "red"
        data2 = [] 
        runner.registerInfo(environment_periods.to_s)
        # put
        environment_periods.each do |environment_p|
          ...
          csv.each_index do |row|
            if row > 0
              ...
              # runner.registerInfo("Mon: #{mon}")
              # runner.registerInfo("Year: #{year}")
              # runner.registerInfo("day: #{day}")            
              dtm = OpenStudio::DateTime.new(dat,tim)
              if !(dtm >= first_date && dtm <= last_date)
                if warning_messages
                  runner.registerInfo("Year: #{year} - Mon: #{mon} - day: #{day} - dat #{dat}")
        ...
        all_series << series
        all_series << series2
        runner.registerInfo("ySum - #{ySum} - n -> #{n}")
    ...
    runner.registerValue("#{csv_var}_nmbe", nmbe, "")
    runner.registerValue("#{csv_var}_cvrmse", cvrmse, "")
